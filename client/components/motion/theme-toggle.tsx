...
export function ThemeToggle({
  ...
  return (
    <button
      ...
      onClick={toggle}
      className={cn(
        "w-8 h-8 rounded-lg border border-border bg-card hover:bg-muted text-foreground flex items-center justify-center transition-colors cursor-pointer shadow-xs",
        className
      )}
      {...rest}
    >
      {mounted ? (
        ...
        >
          {isDark ? (
            <Sun className={cn("text-foreground", iconClassName)} />
          ) : (
            <Moon className={cn("text-foreground", iconClassName)} />
          )}
        </ActionSwapIcon>
      ) : (
